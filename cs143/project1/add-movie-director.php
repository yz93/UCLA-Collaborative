    ...
    <div id="wrap">
      ...
      <div id="main-content" class="container">
        <div class='row'>
          ...
          <hr/>
          <p class='alert alert-info'><span class='glyphicon glyphicon-info-sign'></span>&nbsp;Enter the information below to link a Director with a Movie.</p>
          ...
          <form method="GET" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" id="input-form" class="form-horizontal" role="form">
            ...
            <div class="form-group">
              <label for="did" class="col-md-2 control-label">Director</label>
              <div class="col-md-10">
                <select class="form-control" name="did" id="did">
                  <?php
                    // Query the DB and get info on the returned attrs
                    $directors = mysql_query("SELECT id, first, last FROM Director ORDER BY last;", $dao);
                    
                    while ($d = mysql_fetch_assoc($directors)) {
                      print "<option value='" . $d["id"] . "'>" . $d["last"] . ", " . $d["first"] . "</option>";
                    }
                  ?>
                </select>
              </div>
          ...
          <?php
            ...
            $inputNames = array("mid", "did", "linkmid", "linkdid");
            $inputVals = array();
            $foundVal = false;
            ...
            if (!empty($inputVals["linkdid"])) {
              print
                "<script>" .
                  "$('#did').val('" . $inputVals["linkdid"]  . "')" .
                "</script>";
              return;
            }
