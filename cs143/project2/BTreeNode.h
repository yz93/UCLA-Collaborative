...
class BTNode {
  public:
    /**
     * The maximum number of keys to be stored at each node
     */
	static const int MAX_LEAF_KEYS = 84;
	static const int MAX_NON_LEAF_KEYS = 126;
    ...
    */
	...
	PageId getPid() { return m_pid; }

	/*set pid*/
	void setPid(PageId id){ m_pid = id; }
  ...
  protected:
    ...
    */
    ...
    PageId m_pid;
    bool m_isLeaf;
    RC ptrToKeyHelp (int eid, int*& ptr, int jumpAmt);
	RC ptrToKeyHelp2(int* buf, int eid, int*& ptr, int jumpAmt);
...
class BTLeafNode : public BTNode {
  ...
  private:
   /**
     * Private helper function that sets input ptr to the eid-th key
     * in the node's buffer
     */
    RC ptrToKey (int eid, int*& ptr);
	/*Private helper function that sets input ptr to the eid-th key
     * in buf */
	RC ptrToKey(int* buf, int eid, int*& ptr);
}; 
...
class BTNonLeafNode : public BTNode {
  public:
    BTNonLeafNode (BTNode& b) : BTNode(b) {}
    BTNonLeafNode () {}
